...
double measure_read_time(const std::string& filename, size_t block_size, bool random_access, int iterations = 5) {
    ...
    return calculateMean(iteration_times);
}

int main() {
    try {
        const std::string filename = "test_file.bin";
        const size_t min_file_size = 1 * 1024 * 1024;  
        ...
        results << "File Size (MB),Sequential Mean (s),Random Mean (s)\n";
        ...
        for (size_t size = min_file_size; size <= max_file_size; size *= step_factor) {
            ...
            std::cout << "File Size: " << (size / (1024 * 1024)) << " MB, Seq Mean: " << seq_mean
                      << " s, Rand Mean: " << rand_mean << " s\n";

            results << (size / (1024 * 1024)) << "," << seq_mean << "," << rand_mean << "\n";
        }
